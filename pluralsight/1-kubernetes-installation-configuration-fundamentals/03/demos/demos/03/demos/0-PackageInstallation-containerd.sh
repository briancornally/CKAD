...
less -pplugins.\"io.containerd.grpc.v1.cri\".containerd.runtimes.runc /etc/containerd/config.toml
grep -C3 SystemdCgroup /etc/containerd/config.toml
...
VERSION=1.21.0-00
apt-cache policy kubelet | grep $VERSION
